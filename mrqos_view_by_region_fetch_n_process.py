...
def main():
    ...
    cleanup_command_2 = """ awk 'NR==FNR{id=$1; count[id]+=$8; if(cum[id]<$9){cum[id]=$9;} next}{id=$1; if(count[id]>0){load_ratio=100*$8/count[id];}else{load_ratio=-1;} print $2,$3,$4,$5,$6,$7,$8,$9, load_ratio, cum[id];}' %s %s > %s""" %( os.path.join(local_dir,'temp1.csv'),
                                                                                                                                                                                                        os.path.join(local_dir,'temp1.csv'),
