...
def main():
    ...
    cleanup_command_1 = """ cat %s | tail -n+3 | sort -t"," -k9gr | awk -F, '{id=$1; count[id]+=1; cum_pert[id]+=$NF; if(count[id]<=30){split($1,a,"."); split(a[2],mr,"_"); split(a[3],geo,"_"); split(a[4],net,"_"); if(length(geo[2])==2){print $1, mr[2], geo[2], net[2], $5, $6, $7, $8, $9, $NF, cum_pert[id];}}}' > %s""" % (os.path.join(local_dir,'temp.csv'),
