...
def main():
    ...
    cleanup_command_1 = """ cat %s | tail -n+3 | sort -t"," -k9gr | awk -F, '{id=$1; count[id]+=1; cum_pert[id]+=$NF; if(count[id]<=20){split($1,a,"."); split(a[2],mr,"_"); split(a[3],geo,"_"); split(a[4],net,"_"); if(length(geo[2])==2){print $1, mr[2], geo[2], net[2], $5, $6, $7, $8, $9, $NF, cum_pert[id];}}}' > %s""" % (os.path.join(local_dir,'temp.csv'),
                                                                                                                                                                                                                                                                                                   os.path.join(local_dir,'temp1.csv') )
    cleanup_command_2 = """ awk 'NR==FNR{id=$1; count[id]+=$9; if(cum[id]<$11){cum[id]=$11;} next}{id=$1; if(count[id]>0){load_ratio=100*$8/count[id];}else{load_ratio=-1;} print $2,$3,$4,$5,$6,$7,$8,$9,$10, load_ratio, cum[id];}' %s %s > %s""" %( os.path.join(local_dir,'temp1.csv'),
                                                                                                                                                                                                        os.path.join(local_dir,'temp1.csv'),
