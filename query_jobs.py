...
def main():
    ...
    #mtype = ['score','distance','in_country','in_continent','in_out_ratio','ra_load','cog'];
    ...
    for item in mtype:
        ...
        Nretrial = config.query_retrial
        Ttimeout = config.query_timeout
        # multiple times with timeout scheme
        while ((flag==0) and (count<Nretrial)):
            try:
                with ytt.Timeout(Ttimeout):
